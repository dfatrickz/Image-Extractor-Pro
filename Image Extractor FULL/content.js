(() => {
  ...
  // --- HIGH-RES URL PARSER (GOOGLE & OVERLAY BYPASS) ---
    ...
    if (window.location.hostname.includes("google.")) {
      ...
      if (parentAnchor?.href && parentAnchor.href.includes("imgurl=")) {
        try {
          ...
          console.warn("IEP: Failed to parse Google imgurl");
        }
      }
    }
  ...
  class FloatingExtractorController {
    ...
    cacheElements() {
      this.elements = {
        ...
        btnOpenGuide: this.shadowRoot.getElementById("iepBtnOpenGuide"),
        btnOpenGuideSettings: this.shadowRoot.getElementById("iepBtnOpenGuideSettings"),
        ...
        quickAutoUpgradeToggle: this.shadowRoot.getElementById("iepQuickAutoUpgrade"),
        ...
        blockedDomainsInput: this.shadowRoot.getElementById("iepBlockedDomains"),
    ...
    attachUiEvents() {
      ...
      const handleOpenGuide = () => {
        api.runtime.sendMessage({ type: "IEP_OPEN_GUIDE" });
      };
      this.elements.btnOpenGuide?.addEventListener("click", handleOpenGuide);
      this.elements.btnOpenGuideSettings?.addEventListener("click", handleOpenGuide);
      ...
      this.elements.quickLazyLoadToggle?.addEventListener("change", (event) => {
        if (this.elements.lazyLoadBypassToggle) this.elements.lazyLoadBypassToggle.checked = event.target.checked;
        this.updateFiltersFromInputs();
      });
      ...
      this.elements.lazyLoadBypassToggle.addEventListener("change", () => {
        this.updateFiltersFromInputs();
      });

      this.elements.autoCheckDuplicatesToggle.addEventListener("change", () => {
        this.updateFiltersFromInputs();
      });

      this.elements.stickyToolbarToggle.addEventListener("change", () => {
        this.updateFiltersFromInputs();
      });
      ...
      this.elements.hoverDownloadToggle.addEventListener("change", (event) => {
        // Instantly sync the Quick Setting toggle to match
        if (this.elements.quickFastGrabToggle) {
          this.elements.quickFastGrabToggle.checked = event.target.checked;
        }
        this.updateFiltersFromInputs();
      });
      ...
      this.elements.startMinimizedToggle.addEventListener("change", () => {
        this.updateFiltersFromInputs();
      });

      this.shadowRoot.querySelectorAll(".iep-help-btn").forEach((button) => {
        button.addEventListener("click", (event) => {
          event.preventDefault();
          event.stopPropagation();
          api.runtime.sendMessage({ type: "IEP_OPEN_GUIDE", anchor: button.dataset.anchor });
        });
      });
      ...
      this.elements.blockedDomainsInput?.addEventListener("change", () => {
        this.updateFiltersFromInputs({ rescan: true });
      });
      ...
      this.elements.autoUpgradeToggle.addEventListener("change", (event) => {
        if (this.elements.quickAutoUpgradeToggle) this.elements.quickAutoUpgradeToggle.checked = event.target.checked;
        this.updateFiltersFromInputs();
      });

      this.elements.quickAutoUpgradeToggle?.addEventListener("change", (event) => {
    ...
    render() {
      ...
      if (this.elements.quickAutoUpgradeToggle) this.elements.quickAutoUpgradeToggle.checked = Boolean(this.state.filters.autoUpgradeResolutions);
      ...
      if (this.elements.blockedDomainsInput) {
        this.elements.blockedDomainsInput.value = this.state.filters.blockedDomains || "";
        this.elements.blockedDomainsInput.disabled = this.state.busy;
      }
      ...
      if (this.elements.quickAutoUpgradeToggle) this.elements.quickAutoUpgradeToggle.disabled = this.state.busy;
    ...
    updateFiltersFromInputs(options = {}) {
      ...
      this.state.filters.lazyLoadBypass = Boolean(this.elements.lazyLoadBypassToggle.checked);
    ...
    async updateSurferHoverButton(point) {
      ...
      if (!imageInfo?.targetImg) {
        this.hideSurferHoverButton();
        return;
      }

      let hoverUrl = imageInfo.url;
      const blockedList = this.state.filters.blockedDomains
        ? parseIgnoredRules(this.state.filters.blockedDomains)
        : [];
      if (blockedList.length > 0 && blockedList.some((domain) => hoverUrl.includes(domain))) {
        this.hideSurferHoverButton();
        return;
      }
      const targetImg = imageInfo.targetImg;
      if (!hoverUrl || (hoverUrl.startsWith("data:image") && !window.location.hostname.includes("google."))) {
        hoverUrl = await getBestImageUrl(targetImg);
      }
      ...
      if (!this.state.filters.hoverDownloadEnabled || !hoverUrl) {
        this.hideSurferHoverButton();
        return;
      }

      if (hoverUrl.startsWith("data:image") && !window.location.hostname.includes("google.")) {
        this.hideSurferHoverButton();
        return;
      }

      const rectTarget = targetImg;
      ...
      this.elements.surferHoverButton.dataset.url = hoverUrl;
    ...
    syncFloatingPosition() {
      ...
      const fallbackPanelTop = clamp(clampedFabY + fabHeight + 16, 12, Math.max(12, viewportHeight - panelHeight - 12));
            ...
            <section class="iep-settings-dialog" role="dialog" aria-modal="true" aria-label="FastGrab Image Downloader settings">
              <div class="iep-modal-header">
                <div style="display: flex; align-items: center; gap: 12px;">
                  ...
                  <button id="iepBtnOpenGuideSettings" class="iep-button iep-button-secondary" style="padding: 4px 10px; font-size: 11px;">Open Guide</button>
                </div>
              ...
              <div class="iep-settings-grid settings-horizontal-wrapper">
                <section class="iep-settings-panel">
                  ...
                  <div class="iep-settings-stack">
                    ...
                    <label class="iep-field">
                      <span>Subfolder Name<button class="iep-help-btn" data-anchor="subfolder-name" title="Creates a specific folder for your downloads.">&#63;</button></span>
                    ...
                    <label class="iep-dark-label" style="display: block; margin-top: 10px; font-size: 13px;">File Naming Scheme<button class="iep-help-btn" data-anchor="naming-scheme" title="Keep original names or use numbered sequences.">&#63;</button></label>
                ...
                <section class="iep-settings-panel">
                  ...
                  <div class="iep-settings-stack">
                    ...
                    <label class="iep-field">
                      <span>Theme<button class="iep-help-btn" data-anchor="theme" title="Light, Dark, or System mode.">&#63;</button></span>
                    ...
                    <label class="iep-field">
                      <span>Theme Style<button class="iep-help-btn" data-anchor="theme-style" title="Adjust the visual flair of the gallery.">&#63;</button></span>
                    ...
                    <label class="iep-field">
                      <span>Images per page<button class="iep-help-btn" data-anchor="images-per-page" title="Paginate the gallery for better performance.">&#63;</button></span>
                ...
                <section class="iep-settings-panel">
                  ...
                  <div class="iep-settings-stack" style="margin-top: 8px; border-top: 1px solid var(--iep-border); padding-top: 16px;">
                    <h3 style="text-align: center; margin: 0 0 8px;">Blocked Domains</h3>
                    <p style="text-align: center; margin-bottom: 8px; color: var(--iep-text-soft);">Images from these domains will be completely ignored.</p>
                    <textarea id="iepBlockedDomains" style="width: calc(100% - 16px); min-height: 90px; resize: vertical; font-family: monospace; font-size: 11px; padding: 10px; background: var(--input-bg, var(--iep-surface-strong)); color: var(--input-text, var(--iep-text-main)); border: 1px solid var(--input-border, var(--iep-border-strong)); border-radius: 8px;" placeholder="adzerk.net&#10;ad.doubleclick.net"></textarea>
                  </div>
                </section>
              </div>
            </section>
  ...
  async function collectFromImageNode(image, imageMap, filters, ignoredRules) {
    const imageInfo = getTrueImageUrl(image);
    if (!imageInfo?.url) {
      return;
    }

    const finalUrl = imageInfo.url;
    if (finalUrl.startsWith("data:image") && window.location.hostname.includes("google.")) {
      return;
    }
    ...
    addCandidate(candidates, finalUrl, "source", 98);

    const highResUrl = await getBestImageUrl(imageInfo.targetImg || image);
    if (highResUrl) {
    ...
    await registerCandidates(imageMap, dedupeCandidates(candidates), {
      ...
      blockedDomainsList: parseIgnoredRules(filters.blockedDomains),
      ignoredRules,
...
async function registerCandidates(imageMap, candidates, context) {
    ...
    for (const candidate of candidates) {
      ...
      if (context.blockedDomainsList && context.blockedDomainsList.length > 0) {
        if (context.blockedDomainsList.some((domain) => finalUrl.includes(domain))) continue;
      }
  ...
  window.getTrueFlickrMax = async function(thumbUrl) {
    ...
    // (Even if it is _h or _k, we must fetch to see if an _o exists!)
  ...
  function detectFormat(url, contentType = "") {
    ...
    if (String(url).includes("unsplash.com/")) return "jpg";
  ...
  async function handleContextQuickDownload() {
    ...
    const baseurl = window.iepLastRightClickedUrl || await getBestImageUrl(target);
